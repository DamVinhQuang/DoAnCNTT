									...
									foreach ($getAllProducts as $value) :
										if ($value['id'] == $_GET['id']) : ?>
											<div class="order-col">
												...
												<div style="max-width:440px;"><?php echo number_format($value['price']) ?><sub>đ</sub></div>
											</div>
							</div>
							<div class="order-col">
								<div><strong>TẠM TÍNH:</strong></div>
								<div><strong class="order-cash-total"><?php echo number_format($_SESSION['cart'][$value['id']] * $value['price']) ?><sub>đ</sub></strong></div>
							</div>
							<div class="order-col">
								<div><strong>PHÍ VẬN CHUYỂN:</strong></div>
									...
									?>
								<div><strong class = "ship-cost-view"><?php echo number_format($shipCost) ?><sub>đ</sub></strong></div>						
								
								
							</div>
							<div class="order-col">
								<div><strong>TỔNG:</strong></div>
								<div><strong class="order-total"><?php echo number_format($_SESSION['cart'][$value['id']] * $value['price'] + $shipCost) ?><sub>đ</sub></strong></div>
							</div>
						</div>
			<?php
			endif;
